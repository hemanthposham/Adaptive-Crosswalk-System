...
#        print(end - start)
    ...
    def write(x, batches, results):
        ...
        cv2.rectangle(img, tuple(map(int, c1)), tuple(map(int, c2)), color, 1)
        t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1 , 1)[0]
        c2 = c1[0] + t_size[0] + 3, c1[1] + t_size[1] + 4
        cv2.rectangle(img, tuple(map(int, c1)), tuple(map(int, c2)), color, 1)
        cv2.putText(img, label, (int(c1[0]), int(c1[1]) + 10), font, 0.5, [225,255,255], 1)
    ...
    det_names = pd.Series(imlist).apply(lambda x: os.path.join(args.det, "det_" + os.path.basename(x)))
